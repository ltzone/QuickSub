...
let test_wrapper ?(print=false) n t1 t2  = 
  ...
  Printf.printf "%u\t%B\t%f\t%B\t%f\n" n linear_res (linear_end -. linear_start) complete_res (complete_end -. complete_start)
...
(* prove: mu a. a -> mu b. b -> .... Nat <: mu a. a -> mu b. b -> .... Nat *)
let test2 (n:int) = 
  let t1, t2 = deep_subtyp_gen n Nat Nat in
  test_wrapper n t1 t2

(* prove: Real -> mu a. Real -> ... mu z. Real -> z <:  Nat -> mu a. Nat -> ... mu z. Nat -> z  *)
let test3 (n:int) = 
