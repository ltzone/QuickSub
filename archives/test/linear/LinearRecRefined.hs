...
rem t (Less, pxs)
  | elem 0 pxs    = Just (Less, (map minus1 (delete 0 pxs)))
  | otherwise     = Nothing
rem t ((Equal l), pxs) 
  | elem 0 pxs    = Just (Equal (map minus1 (delete 0 l)), (map minus1 (delete 0 pxs)))
  | otherwise     = Just (Equal (map minus1 (delete 0 l)), ((map minus1 (delete 0 pxs)) \\ (fvars t)))
...
(Less, pv1)        &&& (Less, pv2)          = Just (Less, intersect pv1 pv2)
(Less, pv1)        &&& ((Equal []), pv2)    = Just (Less, intersect pv1 pv2)
((Equal []), pv1)  &&& (Less, pv2)          = Just (Less, intersect pv1 pv2)
((Equal xs), pv1)  &&& ((Equal ys), pv2)    = Just ((Equal (union xs ys)), intersect pv1 pv2)
_           &&& _             = Nothing
...
check x e m TInt       TInt         = Just (eq, [0..(x-1)])
check x e m Top        Top          = Just (eq, [0..(x-1)])
check x e m a          Top          = Just (lt, [0..(x-1)])
check x e m (Var i)    (Var j)
   | i == j && e !! i == m          = Just (eq, [0..(x-1)])
   | i == j && e !! i == neg m      = Just (Equal [i], delete i [0..(x-1)])
